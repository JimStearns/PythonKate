...
from simplishgui import SimplishGui
import random
...
def draw(canvas):
    """By telling simplishgui's draw timer to call method (via set_draw_handler())
        this method will be called about 60 times a second.

        Suggestion: the ball position and velocity updates in timer_handler()
        could be moved into this method, removing the need for a separate timer.
    """
...
pong_gui = SimplishGui()

# create frame
frame = pong_gui.create_frame("Pong", WIDTH, HEIGHT, background=BKGND_COLOR)
...
timer = pong_gui.create_timer(TIMER_INTERVAL_MS, timer_handler)

timer.start()
# Call mainloop last - this method doesn't return until window exited.
frame.mainloop()
print("Good game!")
